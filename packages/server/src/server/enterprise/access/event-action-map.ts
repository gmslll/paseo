...
> = {
  ...
  "collab.stream.token.request": ["workspace.metadata.read"],
  "collab.subscription.poll.request": ["workspace.metadata.read"],
...
export const OUTBOUND_RESOURCE_ACTION_GROUPS: readonly OutboundResourceActionGroup[] = [
  ...
  },
  {
    ...
    events: [
      ...
      "collab.stream.token.response",
      "collab.subscription.poll.response",
      "agent_status",
      "agent_update",
...
export const OUTBOUND_INHERITED_CONTEXT_EVENTS = [
  ...
  "enterprise.browser.page_identity.invalidate.response",
] as const satisfies readonly SessionOutboundMessage["type"][];
