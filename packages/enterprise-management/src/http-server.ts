...
<div class="login"><input id="token" type="password" autocomplete="off" placeholder="管理员或 Boss PAT（只保存在此页面内存）"><button id="login">登录 / 刷新</button><button id="logout" class="secondary">清除</button></div>
...
function renderPlacements(placements){const root=q('#placements');root.replaceChildren(...placements.map((item)=>element('article',{class:'item row between'},[element('div',{},[element('strong',{text:item.resource.resourceKind+' · '+item.resource.localResourceId}),element('div',{class:'muted',text:'owner '+item.ownerPrincipalId})]),element('span',{class:'pill',text:item.resource.nodeId})])));if(!placements.length)root.textContent='节点尚未上报 Workspace / Agent / Browser Profile / App Slot。'}
function renderAudit(events){const root=q('#audit');if(!events.length){root.textContent='暂无可见审计事件。';return}const table=element('table');table.append(element('thead',{},[element('tr',{},['时间','节点','动作','结果','资源','员工'].map((value)=>element('th',{text:value})))]));table.append(element('tbody',{},events.map((event)=>element('tr',{},[event.occurredAt,event.nodeId,event.action,event.outcome,event.resourceKind+':'+event.resourceId,event.actorPrincipalId].map((value)=>element('td',{text:String(value)})))));root.replaceChildren(table)}
