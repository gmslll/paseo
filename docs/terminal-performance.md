...
- **Desktop bridge benchmark (main process → renderer frame path):** `npm run transport-bench --workspace=@getpaseo/desktop`, with `BENCH_ROUNDS_SCALE=5` for decision-grade percentiles. Runs real Electron under the production window constraints (`sandbox` + `contextIsolation`) and races three inbound paths: base64, structured-clone bytes, and a dedicated `MessageChannelMain` port. Writes JSON to `/tmp/paseo-transport-bench/`. Healthy numbers (2026-09, darwin-arm64): a 1MiB frame is p50 ~0.5ms alone and ~1.6ms four ways concurrent, ~2.4GiB/s. base64 costs 10x the p95 and 12x the throughput on 1MiB frames, which is why the bridge carries bytes. A dedicated port was measured and rejected: slower in 8 of 8 size/concurrency cases (p50 1.0–1.4x, large-frame throughput 25–34% lower), because contextBridge cannot hand a `MessagePort` to the main world — preload has to forward every frame on by callback, so the port buys a hop instead of removing one.
