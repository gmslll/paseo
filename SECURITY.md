...
The daemon also serves local planes to tools running as the same user (ADR-0038). `$PASEO_HOME/run` is mode 0700, and its sockets and `local-token` are mode 0600. A new token is written on every start. `run/probe.sock` answers health and status with no Principal, Grant, or credential data. `run/control.sock` carries an ordinary Session and requires the `x-paseo-local-token` header, so reaching the socket is not enough; on an enterprise node it also requires a PAT or Session Ticket, and the token alone admits no one. `POST /v1/rpc` on the same socket runs a single request through an ordinary Session under that same admission, so a one-shot caller reaches nothing a streaming Session could not. `run/terminal.sock` carries the terminal traffic of a Session that is already admitted. Besides the local token it requires a one-use attach token the Session asked for itself, which expires in a minute and names the Session, Principal, Grant version, and plane it was issued for; a Session that arrived over the relay or a Hub is never issued one. The plane carries terminal requests only, so every other RPC stays on the Session's own channel where its response is authorized. `run/data.sock` attaches to a Session on the same terms and carries document frames; it listens only when the daemon is built with a handler that serves documents, so by default there is no data socket at all. When a socket path would exceed 104 bytes the sockets move to `$TMPDIR/paseo-<home digest>/`, and the daemon refuses that directory if it is a symlink or owned by another user.
